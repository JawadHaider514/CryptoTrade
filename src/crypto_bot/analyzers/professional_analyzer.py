...
class ProfessionalAnalyzer:
    ...
    def __init__(self, config=None):
        ...
        Args:
            config: Optional config dict (legacy). If not provided, loads from analyzer_config.json
        """
        # Convert config dict to ConfigObj for attribute access
        ...
        # Load analyzer config from JSON if available
        ...
        self.layer_weights = self.analyzer_config.get("weights", {
            "layer_1": 0.15,
            "layer_2": 0.15,
            "layer_3": 0.12,
            "layer_4": 0.12,
    ...
    def load_analyzer_config(config_path: str = "config/analyzer_config.json") -> Dict[str, Any]:
        """
        Load ProfessionalAnalyzer configuration from JSON
        
        Args:
            config_path: Path to analyzer_config.json from accuracy_validator
        
        Returns:
            Configuration dictionary with thresholds, weights, etc.
        """
        default_config = {
